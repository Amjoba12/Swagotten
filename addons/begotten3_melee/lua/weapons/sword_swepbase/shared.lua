		...
		if string.find(tr.Entity:GetClass(), "prop_ragdoll") then
			...
			if weptype:IsValid() and weptype:GetClass() == "begotten_polearm_lungemine" then
				...
				if weaponItemTable then
					...
					-- Сообщение в чат
					Clockwork.chatBox:AddInTargetRadius(self.Owner, "me", "is torn apart by their own thunderous spear!", self.Owner:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2)
					
					-- Удаление предмета
					self.Owner:TakeItem(weaponItemTable)
					
					-- МОЩНЫЙ ВЗРЫВ ПРИ ЛЮБОМ КОНТАКТЕ!
					-- 1. Взрывная волна с большим радиусом
					...
					explosion:Spawn()
					explosion:Activate()
					explosion:Fire("Explode", "", 0.01)
					
					-- 2. Прямой урон всем в радиусе 500 юнитов
					local radius = 250
					...
					for _, ent in pairs(ents.FindInSphere(pos, radius)) do
						if ent:IsPlayer() or ent:IsNPC() or ent:IsNextBot() then
							...
							local damageScale = 1 - (distance / radius)
							local finalDamage = math.max(100, maxDamage * damageScale)
							
							local damageInfo = DamageInfo()
							damageInfo:SetDamage(finalDamage)
							damageInfo:SetAttacker(self.Owner)
							damageInfo:SetInflictor(weptype)
					...
					-- 3. Убийство владельца
					local selfDamage = DamageInfo()
					selfDamage:SetDamage(9999)
					selfDamage:SetAttacker(self.Owner)
					selfDamage:SetInflictor(weptype)
					selfDamage:SetDamageType(DMG_BLAST)
					selfDamage:SetDamagePosition(self.Owner:GetPos())
					self.Owner:TakeDamageInfo(selfDamage)
					
					-- 4. Визуальные эффекты
					util.ScreenShake(pos, 1000, 500, 2, 1500)
					...
					timer.Simple(5, function() if IsValid(fire) then fire:Remove() end end)
					
					-- Дым
					local smoke = ents.Create("env_smokestack")
					smoke:SetPos(pos + Vector(0,0,50))
					smoke:SetKeyValue("smokematerial", "particles/smokestack")
					smoke:SetKeyValue("basespread", "100")
					smoke:SetKeyValue("spreadspeed", "200")
					smoke:SetKeyValue("speed", "300")
					smoke:SetKeyValue("startsize", "50")
					smoke:SetKeyValue("endsize", "300")
					smoke:SetKeyValue("rate", "20")
					smoke:Spawn()
					smoke:Fire("TurnOn", "", 0)
					timer.Simple(4, function() if IsValid(smoke) then smoke:Remove() end end)
					
					util.Decal("Scorch", pos + VectorRand(), pos - VectorRand())
					self.Owner:EmitSound("fiend/wtinessme.wav", 100, 100)
...
--if (SERVER) then
	function SWEP:HandleHit(hit, src, swingType, hitIndex, offhandWeapon, offhandAttackTable)
		-- =====================================================
		-- THUNDER SPEAR (begotten_polearm_lungemine) MEGA EXPLOSION
		-- =====================================================
		local isThunderSpear = self:GetClass() == "begotten_polearm_lungemine"
		
		if isThunderSpear then
			local pos = src or hit:GetPos()
			local owner = self.Owner
			
			-- Удаление копья из инвентаря
			local weaponItemTable = item.GetByWeapon(self)
			if weaponItemTable then
				owner:TakeItem(weaponItemTable)
			end
			
			-- Сообщение в чат
			Clockwork.chatBox:AddInTargetRadius(owner, "me", "is torn apart by their own thunderous spear!", owner:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2)
			
			-- 1. СОЗДАЁМ МОЩНЫЙ ВЗРЫВ
			local explosion = ents.Create("env_explosion")
			explosion:SetPos(pos)
			explosion:SetOwner(owner)
			explosion:SetKeyValue("iMagnitude", "1000")      -- Огромная сила
			explosion:SetKeyValue("iRadiusOverride", "250")  -- Радиус 500 юнитов
			explosion:Spawn()
			explosion:Activate()
			explosion:Fire("Explode", "", 0.01)
			
			-- 2. ГАРАНТИРОВАННЫЙ УРОН ДЛЯ ВСЕХ В РАДИУСЕ
			local radius = 250
			local maxDamage = 1000  -- Колоссальный урон в эпицентре
			...
			-- 3. УБИВАЕМ ВЛАДЕЛЬЦА (гарантированно)
			local selfDamage = DamageInfo()
			selfDamage:SetDamage(9999)
			selfDamage:SetAttacker(owner)
			selfDamage:SetInflictor(self)
			selfDamage:SetDamageType(DMG_BLAST)
			selfDamage:SetDamagePosition(owner:GetPos())
			owner:TakeDamageInfo(selfDamage)
			
			-- 4. ВИЗУАЛЬНЫЕ ЭФФЕКТЫ
			util.ScreenShake(pos, 1000, 500, 3, 1500)
			
			-- Огонь на месте взрыва
			local fire = ents.Create("env_fire")
			fire:SetPos(pos)
			fire:SetKeyValue("firesize", "128")
			fire:SetKeyValue("fireattack", "0")
			...
			return  -- Выходим, чтобы обычная логика удара не применялась
		end
		-- =====================================================
		-- END OF THUNDER SPEAR CODE
		-- =====================================================
