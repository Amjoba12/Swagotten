...
local COMMAND = Clockwork.command:New("ClearMap")
    COMMAND.tip = "Remove all items, ragdolls, and belongings from the map at once. You can also remove specific object types using arguments. Belongings are always removed last."
    COMMAND.text = "[string Type]"
    COMMAND.access = "s"
    COMMAND.alias = {"ClearAll", "MapCleanup"}
    COMMAND.optionalArguments = 1
    ...
    function COMMAND:OnRun(player, arguments)
        local items_removed = 0
        local ragdolls_removed = 0
        local belongings_removed = 0
        local selection = arguments[1] and string.lower(arguments[1])
        
        -- Determine what to remove
        local remove_items = true
        local remove_ragdolls = true
        local remove_belongings = true
        
        if selection then
            remove_items = false
            remove_ragdolls = false
            remove_belongings = false
            
            if selection == "items" or selection == "1" then
                remove_items = true
            elseif selection == "ragdolls" or selection == "2" then
                remove_ragdolls = true
            elseif selection == "belongings" or selection == "3" then
                remove_belongings = true
            elseif selection == "all" or selection == "0" then
                remove_items = true
                remove_ragdolls = true
                remove_belongings = true
            else
                Schema:EasyText(player, "grey", "Invalid object type. Use: items, ragdolls, belongings, or all.")
                return
            end
        end
        
        -- Remove items first (excluding decoys/trap items)
        ...
        -- Remove ragdolls second (excluding alive players' ragdolls)
        if remove_ragdolls then
            for k, v in pairs(ents.FindByClass("prop_ragdoll")) do
                if Clockwork.entity:IsPlayerRagdoll(v) then
                    local ragdollPlayer = Clockwork.entity:GetPlayer(v)
                    if not ragdollPlayer:Alive() then
                        ragdolls_removed = ragdolls_removed + 1
                        v:Remove()
                    end
                else
        ...
        -- Remove belongings LAST (always)
        if remove_belongings then
            for k, v in pairs(ents.FindByClass("cw_belongings")) do
                belongings_removed = belongings_removed + 1
                v:Remove()
            end
        end
        
        -- Build the message
        local message = player:Name() .. " cleared the map: "
        local parts = {}
        
        if remove_items then
            table.insert(parts, items_removed .. " items")
        end
        
        if remove_ragdolls then
            table.insert(parts, ragdolls_removed .. " ragdolls")
        end
        
        if remove_belongings then
            table.insert(parts, belongings_removed .. " belongings")
        end
        
        message = message .. table.concat(parts, ", ") .. "."
        
        Schema:EasyText(Schema:GetAdmins(), "cornflowerblue", message, nil)
        
        -- Send confirmation to the executor
        if selection then
            local type_name = selection == "1" and "items" or
                            selection == "2" and "ragdolls" or
                            selection == "3" and "belongings" or
                            selection == "all" and "everything" or
                            selection
                            
            Schema:EasyText(player, "cornflowerblue", "You cleared " .. type_name .. " from the map.")
        else
            Schema:EasyText(player, "cornflowerblue", "You cleared everything from the map.")
        end
    end
COMMAND:Register()