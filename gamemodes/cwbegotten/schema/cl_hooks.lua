...
function Schema:ModifyItemMarkupTooltip(category, maximumWeight, weight, condition, percentage, name, itemTable, x, y, width, height, frame, bShowWeight)
	...
	elseif (category == "Armor" or category == "Helms") then
		...
		if itemTable.attributes then
			...
			if table.HasValue(itemTable.attributes, "burninghatred") then
				frame:AddText("Burning Hatred: Equipping will upgrade 'Hatred' to 'Burning Hatred'. Fire damage burns through 'Burning Hatred' rather than health. You have immunity to burn wounds so long as you have 'Burning Hatred' and you gain 15% movespeed while you are on fire. Doubles ignition time from flagellating with a lantern.", Color(110, 30, 30), nil, 0.9);
			end
			
			if table.HasValue(itemTable.attributes, "judgement") then
				frame:AddText("Judgement: Equipping grants the ability to let out a cry for judgement.", Color(110, 30, 30), nil, 0.9);
			end
		...
		--if itemTable.weight then
            ...
            elseif itemTable.weightclass == "Super heavy" then
				frame:AddText("Sprint Speed Reduction: 45%", Color(110, 30, 30), nil, 0.9);
			end
		--end
