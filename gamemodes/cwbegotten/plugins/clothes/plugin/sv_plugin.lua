...
function PLUGIN:ModifyPlayerSpeed(player, infoTable)
	...
	if (clothesItem) then
		if clothesItem.weightclass == "Super heavy" then
			infoTable.runSpeed = infoTable.runSpeed * 0.55;  -- 45% штраф к бегу
		...
		if clothesItem.attributes then
			...
			elseif table.HasValue(clothesItem.attributes, "burninghatred") and player:IsOnFire() then
...
function PLUGIN:GetRollTime(player)
	...
	if (clothesItem) then
		if clothesItem.weightclass == "Super heavy" then
			return 1.3;  -- Ещё медленнее перекат
		elseif clothesItem.weightclass == "Heavy" then
