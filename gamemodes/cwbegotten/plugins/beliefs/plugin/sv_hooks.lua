	...
	if entity:IsPlayer() or entity:IsNPC() or entity:IsNextBot() or entity.isTrainingDummy then
		...
		if entity:IsPlayer() then
			if damageInfo:IsDamageType(DMG_BURN) then
				...
				local clothesItem = entity:GetClothesEquipped(); 
				if entity:GetCharacterData("Hatred") and (clothesItem and clothesItem.attributes and table.HasValue(clothesItem.attributes, "burninghatred")) then
					local hatred = entity:GetNetVar("Hatred", 0);
					local newhatred = math.max(hatred - newDamage, 0);
					
					if !entTab.opponent then
						entity:SetCharacterData("Hatred", newhatred);
					end
					entity:SetLocalVar("Hatred", newhatred);
					if hatred and hatred > 0 then
						newDamage = newDamage - newDamage;
					end
					if (!entity.nextPainSound or entity.nextPainSound < CurTime()) then
						if entity:GetGender() == GENDER_MALE then
							entity:EmitSound("begotten/brazenbull/burningvehiclescreaming"..math.random(2,3)..".ogg", 90, math.random(90,105), 2);
							entity.nextPainSound = CurTime()+6;
						else
							entity:EmitSound("begotten/brazenbull/femalebrazenbull1.mp3", 90, math.random(90,105), 2);
							entity.nextPainSound = CurTime()+9.5;
						end
					end
				end
			end
	...
	if damage > 0 then
		if entity:IsPlayer() and not entTab.cwWakingUp then
            ...
            if clothesItem and clothesItem.attributes and table.HasValue(clothesItem.attributes, "solblessed") then
                if damageInfo:IsDamageType(DMG_BURN) and table.HasValue(clothesItem.attributes, "burninghatred") then
                    return;
                else
		...
		if entity:Health() - damage < 10 then
			...
			if !entTab.scornificationismActive and !entTab.opponent and entity:HasBelief("fortitude_finisher") then
				...
				if (action != "die") and (action != "die_bleedout") then
					...
					Clockwork.player:SetAction(entity, "die", 15, 1, function()
...
-- Снимаем лимит при обычной смерти и респавне (НЕ через команду)
function cwBeliefs:PlayerSpawn(player)
	-- Если игрок респавнится обычным способом (не через команду /respawn)
	-- и у него есть лимит, снимаем его
	if player:GetCharacterData("respawnLevelCap") then
		player:SetCharacterData("respawnLevelCap", nil)
		player:SetLocalVar("respawnLevelCap", nil)
		Clockwork.player:Notify(player, "Your level cap has been removed!")
	end
end