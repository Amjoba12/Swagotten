...
function cwBeliefs:LevelUp(player)
	if IsValid(player) then
        -- ПРОВЕРКА НА ЛИМИТ УРОВНЯ ПОСЛЕ РЕСПАВНА
		local levelCap = player:GetCharacterData("respawnLevelCap")
		local currentLevel = player:GetCharacterData("level", 1)
		
		if levelCap and currentLevel >= levelCap then
			-- Не даём повышать уровень выше лимита
			return
		end
