...
function Clockwork.player:UseCharacter(player, characterID)
	...
	if (currentCharacter != character) then
		...
		if (fault == nil or fault == true) then
			...
			if (character.subfaction == "Kinisger") then
    			local kinisgerCount = 0;
    			local maxKinisger = 2;

    			for _, v in ipairs(_player.GetAll()) do
        			if (v.cwCharacter) then
            			if (v.cwCharacter.subfaction == "Kinisger") then
                			kinisgerCount = kinisgerCount + 1;
            			end
        			end
    			end

    			if (currentCharacter and currentCharacter.subfaction == "Kinisger") then
        			kinisgerCount = kinisgerCount - 1;
    			end

    			if (kinisgerCount >= maxKinisger) then
        			return false, "The limit of Kinisger players has been reached (" .. maxKinisger .. "/" .. maxKinisger .. ")!"
    			end
			end
